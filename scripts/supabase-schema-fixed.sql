...
-- FIXED: Complete Database Schema for Supabase PostgreSQL
-- This version creates contact_submissions table if it doesn't exist
...
DROP POLICY IF EXISTS "Public read active machines" ON vending_machines;
DROP POLICY IF EXISTS "Public read machine images" ON machine_images;
DROP POLICY IF EXISTS "Public read active products" ON products;
DROP POLICY IF EXISTS "Public read active service areas" ON service_areas;
...
DROP POLICY IF EXISTS "Service role full access machines" ON vending_machines;
DROP POLICY IF EXISTS "Service role full access machine images" ON machine_images;
DROP POLICY IF EXISTS "Service role full access products" ON products;
DROP POLICY IF EXISTS "Service role full access service areas" ON service_areas;
DROP POLICY IF EXISTS "Service role full access business info" ON business_info;
DROP POLICY IF EXISTS "Service role full access SEO settings" ON seo_settings;
DROP POLICY IF EXISTS "Service role full access contacts" ON contact_submissions;
DROP POLICY IF EXISTS "Service role full access admin users" ON admin_users;
DROP POLICY IF EXISTS "Service role full access activity log" ON admin_activity_log;
...
BEGIN
  ...
  RAISE NOTICE '  1. vending_machines';
  RAISE NOTICE '  2. machine_images';
  RAISE NOTICE '  3. products';
  RAISE NOTICE '  4. service_areas';
  RAISE NOTICE '  5. business_info';
  RAISE NOTICE '  6. seo_settings';
  RAISE NOTICE '  7. contact_submissions';
  RAISE NOTICE '  8. admin_users';
  RAISE NOTICE '  9. admin_activity_log';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '3. Run data migration: npx tsx scripts/migrate-data.ts';
  RAISE NOTICE '';
END $$;
